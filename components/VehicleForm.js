...
function Section({ title, children }) {
  return (
    <div style={{ marginTop: 22 }}>
      <div className="heading" style={{ fontSize: 14, borderBottom: "2px solid #2FA84F", display: "inline-block", paddingBottom: 3 }}>{title}</div>
...
export default function VehicleForm({ initial, stockNumber }) {
  ...
  const submit = async () => {
    ...
    if (!payload.chassis_number) payload.chassis_number = null;
  ...
  return (
    <div className="container-narrow" style={{ padding: "24px 20px" }}>
      ...
      <Section title="Photos">
        <div style={{ gridColumn: "1 / -1" }}>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {(v.photos || []).map((p, i) => (
              <div key={i} style={{ position: "relative", width: 84, height: 64, borderRadius: 4, overflow: "hidden", border: i === 0 ? "2px solid #2FA84F" : "1px solid #E4E6EA" }}>
      ...
      {err && <div style={{ color: "#2FA84F", fontSize: 13, marginTop: 16 }}>{err}</div>}
