    ...
    <div className="container" style={{ padding: "24px 20px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 28, marginTop: 14 }}>
        <div>
          ...
          {photos.length > 1 && (
            <div style={{ display: "flex", gap: 6, marginTop: 8, overflowX: "auto" }}>
              {photos.map((p, i) => (
                <button key={i} onClick={() => setImgIdx(i)} style={{ width: 56, height: 42, flexShrink: 0, borderRadius: 3, overflow: "hidden", border: i === imgIdx ? "2px solid #2FA84F" : "1px solid #E4E6EA", padding: 0, cursor: "pointer" }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        <div>
          ...
          <div className="heading" style={{ fontSize: 22, color: "#2FA84F", marginTop: 6 }}>{v.selling_price ? money(v.selling_price) : "Price on request"}</div>
